...
COPERNICUS_SEARCH_URL = "https://catalogue.dataspace.copernicus.eu/stac/collections/SENTINEL-2/items"
...
async def buscar_imagenes(
    ...
    # Nueva API STAC (OData descomisionado en marzo 2026)
    params = {
        "bbox": f"{min_lon},{min_lat},{max_lon},{max_lat}",
        "datetime": f"{fecha_inicio}T00:00:00Z/{fecha_fin}T23:59:59Z",
        "limit": "10",
        "sortby": "-datetime",
        "filter": f"eo:cloud_cover<={int(max_nubosidad)}",
    ...
    try:
        ...
        for item in data.get("features", []):
            ...
            if "PRODUCT" in assets:
                size_mb = round(assets["PRODUCT"].get("file:size", 0) / 1e6, 1)
            productos.append({
                "id": nombre,
                "nombre": nombre,
                "fecha": fecha_img,
